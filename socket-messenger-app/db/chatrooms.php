<?php 
	...
	class chatrooms
	{
		...
		public function getSpecificUserChatRoom() {
			$stmt = $this->dbConn->prepare("SELECT c.*, u.name FROM chatrooms c JOIN users u ON(c.userid = u.id) AND c.userid = ".$this->receiverId." ORDER BY c.id DESC");
			$stmt->execute();
			$chatrooms = $stmt->fetchAll(PDO::FETCH_ASSOC);
			return $chatrooms;
		}
